import {
  ...
  ElementRef, ViewChild, signal, computed, AfterViewInit
} from '@angular/core';
    ...
    <div class="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-slate-500 text-sm">Visualización del DAG — arrastra los nodos para reorganizar</p>
        </div>
...
export class VistaGrafoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  tooltip = signal<{
    x: number; y: number; nombre: string; codigo: string;
    estado: string; creditos: number; prereqs: string[];
  } | null>(null);
  ...
  private buildGraph() {
    ...
    const link = g.append('g').selectAll('line')
      ...
      .attr('class', d => {
        const src = materias.find(m => m.id === (d.source as GraphNode).id
          || m.id === d.source);
    ...
    // Name below node
    node.append('text')
      .text(d => d.materia.nombre.length > 16
        ? d.materia.nombre.substring(0, 14) + '…'
        : d.materia.nombre)
      .attr('text-anchor', 'middle')
    ...
    node
      ...
      .on('mousemove', (event) => {
        ...
        if (t) this.tooltip.set({ ...t, x: event.clientX - rect.left + 12, y: event.clientY - rect.top - 10 });
      })
    ...
    this.simulation = d3.forceSimulation<GraphNode>(nodes)
      .force('link', d3.forceLink<GraphNode, GraphLink>(links)
        .id(d => d.id).distance(100).strength(0.8))
      .force('charge', d3.forceManyBody().strength(-280))
      .force('center', d3.forceCenter(W / 2, H / 2))
      .force('y', d3.forceY<GraphNode>(d => (d.materia.semestre / 9) * H * 0.8 + H * 0.1).strength(0.5))
      .force('x', d3.forceX(W / 2).strength(0.05))
      .force('collision', d3.forceCollide<GraphNode>(d => 14 + d.materia.creditos * 1.5 + 10))
      .on('tick', () => {
        link
  ...
  cerrarModal() { this.materiaSeleccionada.set(null); }
